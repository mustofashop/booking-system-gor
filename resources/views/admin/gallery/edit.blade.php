@extends('layout.dashboard.app', ['title' => 'Edit Gallery'])

@section('content')

<section class="section">
    @foreach ($label as $item)
    @if ($item->code == 'galeri.edit')
            ...
            <div class="card border-0 shadow rounded">
                <div class="card-header">
                    ...
                    <div class="card-header-action">
                        <a href="{{ route('gallery.index') }}" class="btn btn-warning" data-toggle="tooltip"
                           title="Back"><i class="fas fa-backward"></i></a>
                    </div>
                </div>
                <div class="card-body">
                    <form id="fm-artc" action="{{ route('gallery.update', $data->id) }}" method="POST"
                        ...
                        <div class="row">
                            ...
                            <div class="col-4">
                                <div id="image-preview" class="image-preview"
                                     data-image-url="{{ isset($user->image) ? asset('storage/gallery/' . $user->image) : '' }}">
                                    <img id="preview"
                                         src="{{ isset($data->image) ? asset('storage/gallery/' . $data->image) : '' }}"
...
<script>
    ...
    var imageUrl = '{{ isset($data->image) ? asset("storage/gallery/" . $data->image) : "" }}';
    if (imageUrl) {

...
@section('content')
    ...
    <style>
        ...
        .member-pic {
            flex: 1;
            /* Make the image occupy the available space */
            overflow: hidden;
        ...
        .member-info {
            flex: 1;
            /* Make the member info occupy the available space */
            display: flex;
            flex-direction: column;
        }
                            ...
                            @foreach ($button as $item)
                                @if ($item->code == 'register')
                                    <a href="{!! html_entity_decode($item->url) !!}"
                                        class="btn-get-started scrollto">{!! html_entity_decode($item->title) !!}</a>
                                @endif
                            @endforeach
                        </div>
                    </div>
                                ...
                                <div class="event-details">
                                    ...
                                    <div class="event-info">
                                        <span> <b> Expiry Date :</b>
                                            {{ date('d F Y', strtotime($event->expiry_date)) }}</span>
                                    </div>
                                            ...
                                            @foreach ($button as $item)
                                                ...
                                                @if ($item->code == 'maps')
                                                    <a target="_blank" href="{{ $event->maps }}"
                                                        class="btn btn-outline-dark btn-block">{!! html_entity_decode($item->title) !!}</a>
                                                @endif
                                            @endforeach
                                        @elseif($event->status == 'INACTIVE')
                                            @foreach ($button as $item)
                    ...
                    <div class="row align-items-center news-block no-gutters" data-aos="fade-up">
                        @if ($value->id % 2 == 1)
                            ...
                            <div class="col-md-8 pt-2">
                                <h3>{{ $value->title }}</h3>
                                <p>{{ date('d F Y H:i', strtotime($value->created_at)) }} |
                                    {{ strtoupper($value->created_by) }}</p>
                            ...
                            <div class="member member-card" data-aos="zoom-in" data-aos-delay="300">
                                <div class="pic">
                                    @if ($value->image && Storage::exists('public/rider/' . $value->image))
                                        <img src="{{ asset('storage/rider/' . $value->image) }}" class="img-fluid"
                                            alt="">
                                    @else
                                        <img src="{{ asset('assets/img/avatar/avatar-5.png') }}" class="img-fluid"
                                            alt="">
                                    @endif
                                </div>
                                <div class="member-info">
                                    ...
                                    <span>{{ strtoupper($value->place) }},
                                        {{ date('d F Y', strtotime($value->date)) }}</span>
                        ...
                        @foreach ($question as $value)
                            <li data-aos="fade-up" data-aos-delay="100">
                                <i class="bx bx-help-circle icon-help"></i> <a data-bs-toggle="collapse" class="collapse"
                                    data-bs-target="#faq-list-{{ $value->id }}"> {{ $value->question }} <i
                        ...
                        @foreach ($testimonial as $value)
                            <div class="swiper-slide">
                                <div class="testimonial-item">
                                    <img src="{{ asset('storage/testimoni/' . $value->image) }}" class="testimonial-img"
